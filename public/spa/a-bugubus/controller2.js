...
app
    ...
    .controller('Sight_recommend_list', function($rootScope, $scope, $selectCity, $window) {
        $scope.check=true;
        //在缓存中查询地址
        if($window.localStorage.getItem('cityKEY')==null){
            ...
            //取值
            var saveCity=$window.localStorage.getItem('cityKEY');
            $scope.positionCity=saveCity;

        }
        //存值
        $scope.selectcity=function (cityKEY,cityvalue){
            $scope.check=false;
            $window.localStorage.setItem('cityKEY',cityvalue);
    ...
    .controller('Sight_details_list', function($rootScope, $scope, $ionicSlideBoxDelegate, $state, $ionicViewSwitcher, $location) {
        $ionicSlideBoxDelegate.$getByHandle('slide').loop(true);
    })

    /*订单支付控制器*/
    .controller('Sight_pay', function($scope) {
        $scope.date=new Date();
    })
